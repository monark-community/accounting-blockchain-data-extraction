...
import { TrendingUp, Search, Filter, Calendar, Eye } from "lucide-react";
...
const IncomeTab = ({ transactions, connectedWallets, getWalletName }: IncomeTabProps) => {
  ...
  const totalIncome = filteredTransactions.reduce((sum, tx) => sum + tx.usdValue, 0);
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <input
                        type="checkbox"
                        id="income-network-all"
                        checked={selectedNetworks.includes("all")}
                        onChange={(e) => handleNetworkChange("all", e.target.checked)}
                        className="rounded"
                      />
                      <label htmlFor="income-network-all" className="text-sm">All Networks</label>
                    </div>
                    {networks.map((network) => (
                      <div key={network} className="flex items-center space-x-2">
                        <input
                          type="checkbox"
                          id={`income-network-${network}`}
                          checked={selectedNetworks.includes(network)}
                          onChange={(e) => handleNetworkChange(network, e.target.checked)}
                          className="rounded"
                        />
                        <label htmlFor={`income-network-${network}`} className="text-sm capitalize">
                          {network}
                        </label>
                      </div>
                    ))}
