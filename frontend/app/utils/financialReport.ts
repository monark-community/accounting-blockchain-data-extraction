...
// Ensure strings are safe inside inline <script> blocks
const sanitizeForScript = (value: string): string =>
  value.replace(/<\/script/gi, "<\\/script>").replace(/<!--/g, "<\\!--");
...
export function generateFinancialReport(
  ...
  const safeExportLabel = escapeHtml(exportLabel);
  const safeDateRangeLabel = escapeHtml(dateRangeLabel);
  const safeWalletInfo = escapeHtml(walletInfo);
  const safeWalletAddresses = escapeHtml(walletAddresses);
  ...
  const safeCategoryChartData = categoryChartData.map((d) => ({
    ...d,
    name: sanitizeForScript(d.name),
  }));
  const safeNetworkChartData = networkChartData.map((n) => ({
    ...n,
    name: sanitizeForScript(n.name),
  }));
  const safeMonthlyArray = monthlyArray.map((m) => ({
    ...m,
    month: sanitizeForScript(m.month),
  }));
...
<head>
  ...
  <title>Financial Report - ${safeExportLabel}</title>
...
<body>
  <div class="container">
    ...
    <div class="header-info">
      <p><strong>Wallet(s):</strong> ${safeWalletInfo}</p>
      <p><strong>Wallet Address(es):</strong> ${safeWalletAddresses}</p>
      <p><strong>Period:</strong> ${safeDateRangeLabel}</p>
    ...
    <div class="footer">
      ...
      <p>Report ID: ${escapeHtml(nowStamp())}</p>
    </div>
  </div>
  ...
  <script>
    ...
    new Chart(categoryCtx, {
      type: 'pie',
      data: {
        labels: ${JSON.stringify(safeCategoryChartData.map(d => d.name))},
        datasets: [{
          data: ${JSON.stringify(safeCategoryChartData.map(d => d.value))},
    ...
    new Chart(monthlyCtx, {
      type: 'line',
      data: {
        labels: ${JSON.stringify(safeMonthlyArray.map(m => m.month))},
        datasets: [{
          label: 'Income',
          data: ${JSON.stringify(safeMonthlyArray.map(m => m.income))},
          borderColor: '#10b981',
          backgroundColor: 'rgba(16, 185, 129, 0.1)',
          tension: 0.4
        }, {
          label: 'Expenses',
          data: ${JSON.stringify(safeMonthlyArray.map(m => m.expense))},
          borderColor: '#ef4444',
          backgroundColor: 'rgba(239, 68, 68, 0.1)',
          tension: 0.4
        }, {
          label: 'Net',
          data: ${JSON.stringify(safeMonthlyArray.map(m => m.net))},
          borderColor: '#3b82f6',
    ...
    new Chart(networkCtx, {
      type: 'bar',
      data: {
        labels: ${JSON.stringify(safeNetworkChartData.map(n => n.name))},
        datasets: [{
          label: 'Inflow',
          data: ${JSON.stringify(safeNetworkChartData.map(n => n.in))},
          backgroundColor: '#10b981'
        }, {
          label: 'Outflow',
          data: ${JSON.stringify(safeNetworkChartData.map(n => n.out))},
...
function generatePDFReport(html: string, data: FinancialReportData, format: "pdf" | "quickbooks" = "pdf"): void {
  ...
  if (format === "pdf") {
    ...
    // Wait for content to load, then trigger print (avoid document.write XSS sink)
